...
// TARGET_CONDI Conditionred = {0, 240, 0, 240, 210, 240, 15, 15, 240, 240}; // 白色   API参数  hsl的阈值，识别时用的
// TARGET_CONDI Conditionred = {0, 15, 40, 255, 15, 230, 15, 15, 240, 120}; // 红色   API参数  hsl的阈值，识别时用的
TARGET_CONDI Conditionred = {60, 100, 40, 255, 15, 200 , 15, 15, 240, 120}; // 绿色   API参数  hsl的阈值，识别时用的
RESULT Result;
...
void DCMI_IRQHandler(void)
{
	if (DCMI_GetITStatus(DCMI_IT_FRAME) == SET) // 捕获到一帧图像
	{
		...
		trace_result = Trace(&Conditionred, &Result);
		printf("trace_result:%d\r\n", trace_result);
		if (trace_result)
		{
			LCD_DrawRectangle(Result.x - Result.w / 2, Result.y - Result.h / 2, Result.x + Result.w / 2, Result.y + Result.h / 2, 0x05ff);
		}
